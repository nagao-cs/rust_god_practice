    ...
    loop {
        ...
        match next {
            ...
            Instruction::Split(addr1, addr2) => {
                if eval_depth(inst, line, is_head, *addr1, sp)? ||
                    eval_depth(inst, line, is_head, *addr2, sp)? {
                        return Ok(true);
                } else {
                    return Ok(false);
                }
...
// 幅優先探索の実装を追加
fn eval_width(inst: &[Instruction], line: &[char], is_head: bool) -> Result<bool, EvalError> {
...
/// 
pub fn eval(inst: &[Instruction], line: &[char], is_head: bool, is_depth: bool) -> Result<bool, EvalError> {
    if is_depth {
        eval_depth(inst, line, is_head, 0, 0)
    } else {
        eval_width(inst, line, is_head)
    }
}